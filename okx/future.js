...
const { okx_socket_base_url } = require("../config")
const book_transformer = require("../helper/book_transformer")
const book_maintainer = require("../helper/book_maintainer")

const socket_name = "OKX Future Market Price Socket"
let ping_interval
const future_symbols = process.env.OKX_FUTURE_SYMBOLS.toString().trim().split(" ") || []
const topics = process.env?.OKX_FUTURE_TOPIC?.toString().trim().split(" ") || []

let all_books = {}

const future_price = () => {

    const socket = new WebSocket(okx_socket_base_url)

    socket.once("open", () => {
        console.log(`${socket_name} Opened.`)

        ping_interval = setInterval(() => {
            socket.send("ping", _error => {
                if (_error) {
        ...
        future_symbols.forEach(_future_symbol => subscribing_param.push({
            "channel": "books",
            "instId": _future_symbol
        }))
    ...
    socket.on("message", (_data, _is_binary) => {
        ...
        if (Buffer.from(_data).toString() === "pong") {
            return
        }
        ...
        try {
            ...
            if (data?.arg?.instId) {
                ...
                if (topic_to_store > -1 && data?.action) {

                    if (data.action === "snapshot") {

                        const book_to_store = book_transformer({ a: data.data[0].asks, b: data.data[0].bids })
                        ...
                        const book_to_store = book_maintainer(all_books[future_symbol], { a: data.data[0].asks, b: data.data[0].bids })
                        all_books[future_symbol] = book_to_store
                        global.APP_EVENTS.emit("future_price", {
                            topic: topics[topic_to_store],
                            message: book_to_store
